...
with tf.Session() as sess:
    ...
    for x in reader[:]:
        ...
        yip = yip.tolist()
        (loss_val, _) = sess.run([loss, train], {p_input: yip})
        print('iter %d:' % (count), loss_val)
        (_input_, _output_,__,kyu) = sess.run([input_, output_,enc_state,dec_state], {p_input: yip})
        #print('train result :')
        #print('enc_state',__)
        #print('dec_state',kyu)
        #print('input :', _input_)
        #print('output :', _output_)
    saver.save(sess, '/home/rijul/Desktop/Model/my_test_model-512',global_step=count)
